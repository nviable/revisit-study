...
describe('Task B methods layout', () => {
  it('uses a 2x2 methods grid through 1600px and four columns above that', () => {
    ...
    expect(css).toMatch(/@media \(min-width: 48em\) and \(max-width: 1600px\)/);
    expect(css).toMatch(/@media \(min-width: 1601px\)[\s\S]*repeat\(4,/);
  });
});
